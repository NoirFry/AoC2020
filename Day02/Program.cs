...
namespace Day02
{
    ...
    {
        private const string Input = @"6-7 z: dqzzzjbzz
...
11-16 r: rrrrrrrrrrrrrrrlr";
        ...
        private static void Main()
        {
            ...
            foreach (string? strPolicy in inputSplitted)
            {
                passPolicy = ParseInput(strPolicy);
                if (passPolicy.IsPassValidPart1())
                    validCountPart1++;
                if (passPolicy.IsPassValidPart2())
                    validCountPart2++;
            }

            Console.WriteLine($"Part1: valid policies: {validCountPart1}");
            Console.WriteLine($"Part2: valid policies: {validCountPart2}");
            Console.ReadKey();
        }

        private static PassPolicy ParseInput(string str)
        {
            string[] strSplitted = str.Split(new[] { ' ' }, StringSplitOptions.None);

            string[] strNumbers = strSplitted[0].Split(new[] { '-' }, StringSplitOptions.None);

            int min = Convert.ToInt32(strNumbers[0], CultureInfo.InvariantCulture);
            int max = Convert.ToInt32(strNumbers[1], CultureInfo.InvariantCulture);
            char letter = strSplitted[1][0];
            string pass = strSplitted[2];

            PassPolicy result = new PassPolicy(min, max, letter, pass);

            return result;
        }
    }

    internal class PassPolicy
    {
        ...
        public PassPolicy(int min, int max, char letter, string pass)
        {
            Min = min;
            Max = max;
            Letter = letter;
        ...
        public bool IsPassValidPart2()
        {
            return Pass[Min - 1] == Letter ^ Pass[Max - 1] == Letter;
        }
    }
}
